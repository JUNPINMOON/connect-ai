...
const ALLOWED_STATUSES = new Set(["active", "draft", "blocked", "archived"]);
...
function main() {
  ...
  if (!["write", "synthetic", "append"].includes(command)) {
    console.log("Usage: node scripts/vault-writer.js synthetic|write|append --memory-root <vault> --storage-root <store> --rel-path <path.md> --type evidence --status draft --project \"Connect AI\" --owner codex --source test --link \"[[00_MOC/AI Agent OS]]\" --body \"...\" [--dry-run]");
    return;
  }
  if (command === "append") {
  ...
  const result = writeDurableNote({
    ...
    body: getArg("body", "# Synthetic Vault Write\n\n- evidence: generated by vault-writer"),
